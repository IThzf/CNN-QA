...
class generate_network_ready_files():
    def __init__(self,word2vec_path,processed_data_path,is_test_data,word_vec_size,max_q_length,max_option_length,max_opt_count,max_sent_para,max_words_sent,op_path=None):
        ...
        # "../data/train/processed_data/text_question_sep_files"
...
#                                    print("s: ",s)
                                ...
                                if len(raw_data_content) == 0:
                                    raw_data_content = "error"    
                                    error_questions.append(question_dir)
                                print("error_error: ",question_dir)
                        else:
                            is_closest_para_file = False
                            raw_data_content = f.readlines()
#                    print("raw_data_content: ",raw_data_content)
                    f = open(os.path.join(op_l_dir, question_dir, fname[:-4]+".pkl"),"w")
                    self.write_vecs_to_file(None,raw_data_content,f,is_correct_answer_file,is_closest_para_file)
                    f.close()
            print 20*"***"
        print "saving final unknown word2vec dictionary to file"
        print("error_questions_len:",len(error_questions))
...
processed_data_path = "../data/train/processed_data"

network = generate_network_ready_files(word2vec_path,processed_data_path,is_test_data=False,word_vec_size=300,max_q_length=10,max_option_length=10,max_opt_count=10,max_sent_para=10,max_words_sent=20,op_path=None)

network.generate_word2vec_for_all()
